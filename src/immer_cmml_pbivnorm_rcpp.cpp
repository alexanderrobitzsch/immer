...
double immer_dnorm2(double a, double b, double rho)
{
	...
	val = exp(-val) / ( 2.0 * pi0 * sqrt( 1 - rho*rho) ); 
	return val;
}
///********************************************************************
...
{
	double pi0 = 3.14159265359;
	double val = exp( - a*a / 2.0 ) / ( sqrt( 2*pi0 ) );       
	return val;
}
///********************************************************************
...
{
	double val=0; 
	double sd1 = sqrt(var1);
	double sd2 = sqrt(var2);     
	double temp1 = cov12_der / sd1 / sd2;
	val = val + temp1;
	temp1 = -0.5 * var1_der * cov12 / pow( sd1, 3.0) / sd2;
	val = val + temp1;
	temp1 = -0.5 * var2_der * cov12 / pow( sd2, 3.0) / sd1;
	val = val + temp1;
	return val;
}
		...
		int N_GAM, Rcpp::IntegerMatrix rho_index, int N_rho ) 
{
	...
	for (int ii=0; ii<I; ii++){
		for (int kk=1; kk<K-1;kk++){
			val1 = - 0.5 * GAM(ii,kk) * pow( var_item[ii], -1.5 );
			//* derivatives LAM        
			for (int dd=0; dd<D; dd++){                  
		...
		Rcpp::NumericVector do_log_par )
{
	...
	for (int ww=0; ww<NR; ww++){
		...
		if ( do_log_par[0] == 1){
			par_fullvec[ww] = exp( par_fullvec[ww] );
		}
	}	
	// compute matrix
...
{
	double pi0 = 3.14159265359; 
	double val = -log(2*pi0);
	double det1 = var1*var2 - cov12*cov12;
	val += - 0.5*log(det1); 
	double val1 = var2 * pow( x - mu1, 2.0) - 2 * cov12 * ( x- mu1)*(y-mu2) + var1*pow( y-mu2, 2.0);
	val += - 0.5*val1 / det1 ;    
	return val;
}
///********************************************************************
		...
		double var2, double cov12 )
{
	...
	val += - 0.5*val1 / det1 ;
	double logdens = val;
	double det1_squared = pow( det1, 2.0);
